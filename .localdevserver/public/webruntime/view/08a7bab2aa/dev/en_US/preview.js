Webruntime.moduleRegistry.define('webruntimegenerated/view__preview', ['lwc', 'webruntime/componentService'], function (lwc, componentService) { 'use strict';
    ...
    **/
    ...
    async function getComponentMetadata(jsName, packageKey) {
      return new Promise(async (resolve, reject) => {
        ...
        if (pkg) {
          ...
          reject(new Error(`Unable to find package ${packageName || 'default'}`));
        }
      });
    }
    ...
    class Preview extends lwc.LightningElement {
      ...
      set cmp(jsName) {
        ...
        setTimeout(() => {
          componentService.createElement(jsName).then(el => {
      ...
      get vscodeHref() {
        return this.metadata && this.metadata.path ? `vscode://file/${this.metadata.path}` : 'javascript:void(0);';
      }
    ...
    var _localdevserverPreview = lwc.registerComponent(Preview, {
      tmpl: _tmpl$7
    });
    ...
    function tmpl$8($api, $cmp, $slotset, $ctx) {
      ...
      return [api_custom_element("localdevserver-preview", _localdevserverPreview, {
        props: {
          "cmp": $cmp.attributes.localdevserverpreview_0.cmp
        },
        key: 2
      }, [])];
    }

    var html = lwc.registerTemplate(tmpl$8);
    tmpl$8.stylesheets = [];
    tmpl$8.stylesheetTokens = {
      hostAttribute: "webruntimegenerated-view__preview_view__preview-host",
      shadowAttribute: "webruntimegenerated-view__preview_view__preview"
    };

    var view__preview = {
      "html": html,
      "attributes": cmp => ({
        "localdevserverpreview_0": {
          "cmp": cmp.routeParams["cmp"] || ''
        }
      })
    };

    return view__preview;
